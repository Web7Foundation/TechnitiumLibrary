...
namespace TechnitiumLibrary.Net.Dns.ResourceRecords
{
    public class DnsDIDATHNRecord : DnsResourceRecordData
    {
        ...
        public override void SerializeTo(Utf8JsonWriter jsonWriter)
        {
            ...
            jsonWriter.WriteString("Tag", _didathnTag);
            jsonWriter.WriteString("DID", _didathnDID);
            jsonWriter.WriteString("Type", _didathnType);
            jsonWriter.WriteString("AuthenticationPublicKey", _didathnAuthenticationPublicKey);
            jsonWriter.WriteString("ControllerDID", _didathnControllerDID);
        ...
        public override ushort UncompressedLength
        {
            get
            {
                ushort tagLength = Convert.ToUInt16(Convert.ToInt32(Math.Ceiling(_didathnTag.Length / 255d)) + _didathnTag.Length);
                ushort didLength = Convert.ToUInt16(Convert.ToInt32(Math.Ceiling(_didathnDID.Length / 255d)) + _didathnDID.Length);
                ushort typeLength = Convert.ToUInt16(Convert.ToInt32(Math.Ceiling(_didathnType.Length / 255d)) + _didathnType.Length);
                ushort authpubkeyLength = Convert.ToUInt16(Convert.ToInt32(Math.Ceiling(_didathnAuthenticationPublicKey.Length / 255d)) + _didathnAuthenticationPublicKey.Length);
                ushort controllerLength = Convert.ToUInt16(Convert.ToInt32(Math.Ceiling(_didathnControllerDID.Length / 255d)) + _didathnControllerDID.Length);
                return (ushort)(tagLength + didLength + typeLength + authpubkeyLength + controllerLength);
            }
        }
        #endregion
    }
